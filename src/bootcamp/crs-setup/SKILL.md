---
name: "crs-setup"
description: Sets up Courses & Bootcamp Suite module in a project. Use when the user requests to 'install crs module', 'configure Courses & Bootcamp Suite', or 'setup Courses & Bootcamp Suite'.
---
...
- **`{project-root}/.ssconfig/config.yaml`** — shared project config: core settings at root (e.g. `output_folder`, `document_output_language`) plus a section per module with metadata and module-specific values. User-only keys (`user_name`, `communication_language`) are **never** written here.
- **`{project-root}/.ssconfig/config.user.yaml`** — personal settings intended to be gitignored: `user_name`, `communication_language`, and any module variable marked `user_setting: true` in `./assets/module.yaml`. These values live exclusively here.
- **`{project-root}/.ssconfig/module-help.csv`** — registers module capabilities for the help system.
...
2. Check if `{project-root}/.ssconfig/config.yaml` exists — if a section matching the module's code is already present, inform the user this is an update
3. Check for per-module configuration at `{project-root}/.ssconfig/crs/config.yaml` and `{project-root}/.ssconfig/core/config.yaml`. If either file exists:
   - If `{project-root}/.ssconfig/config.yaml` does **not** yet have a section for this module: this is a **fresh install**. Inform the user that installer config was detected and values will be consolidated into the new format.
   - If `{project-root}/.ssconfig/config.yaml` **already** has a section for this module: this is a **legacy migration**. Inform the user that legacy per-module config was found alongside existing config, and legacy values will be used as fallback defaults.
...
```bash
python3 ./scripts/merge-config.py --config-path "{project-root}/.ssconfig/config.yaml" --user-config-path "{project-root}/.ssconfig/config.user.yaml" --module-yaml ./assets/module.yaml --answers {temp-file} --legacy-dir "{project-root}/.ssconfig"
python3 ./scripts/merge-help-csv.py --target "{project-root}/.ssconfig/module-help.csv" --source ./assets/module-help.csv --legacy-dir "{project-root}/.ssconfig" --module-code crs
```
...
After both merge scripts complete successfully, remove the installer's package directories. Skills and agents in these directories are already installed at `.claude/skills/` — the `.ssconfig/` directory should only contain config files.

```bash
python3 ./scripts/cleanup-legacy.py --bmad-dir "{project-root}/.ssconfig" --module-code crs --also-remove _config --skills-dir "{project-root}/.claude/skills"
```
