...
# dependencies = []
# ///
"""First Breath — Deterministic sanctum scaffolding for crs-agent-manager.

This script runs BEFORE the conversational awakening. It creates the sanctum
folder structure, the shared project memory structure, copies template files
with config values substituted, copies all capability files and their
supporting references into the sanctum, and auto-generates CAPABILITIES.md
...
Example:
    uv run scripts/init-sanctum.py /Users/me/myproject /path/to/crs-agent-manager
...
def initiate_project_memory(project_memory_path: Path) -> None:
    """Create the shared project memory structure for all crs agents."""
    # Create directory structure
    project_memory_path.mkdir(parents=True, exist_ok=True)
    (project_memory_path / "daily").mkdir(exist_ok=True)
    (project_memory_path / "curated").mkdir(exist_ok=True)
    (project_memory_path / "curated" / "content-drafts").mkdir(exist_ok=True)

    # Initialize index.md if it doesn't exist
    index_path = project_memory_path / "index.md"
    if not index_path.exists():
        index_content = """# Courses & Bootcamp Suite — Project Index

## Current Course Status

**Status:** No active course
**Phase:** —
**Last Updated:** —

## Active Course

_This section is populated when a course is started via PDLC Orchestration._
...
- Start a new course with Discovery phase
"""
        index_path.write_text(index_content)
        print(f"  Created project memory index at {index_path}")
    else:
        print(f"  Project memory index already exists at {index_path}")

    # Initialize curated files with placeholders
    curated_files = {
        "discovery-log.md": "# Discovery Log\n\n_Vision parameters locked during Discovery phase._\n\n_No entries yet. Start a Discovery session with the Consultant._\n",
        "knowledge-base.md": "# Knowledge Base\n\n_Validated research data and technical references._\n\n_No entries yet. Populated by crs-agent-researcher during Design phase._\n",
        "curriculum-design.md": "# Curriculum Design\n\n_Curriculum structure, timeline, engagement map._\n\n_No entries yet. Populated by crs-agent-architect during Design phase._\n",
        "instructor-profile.md": "# Instructor Profile\n\n_Tone of voice, delivery style, and presentation preferences._\n\n_No entries yet. Populated by crs-agent-delivery during Development phase._\n",
        "simulation-reports.md": "# Simulation Reports\n\n_Student simulation results and quality audit reports._\n\n_No entries yet. Populated by crs-agent-simulator during Testing phase._\n",
    }

    for filename, content in curated_files.items():
        file_path = project_memory_path / "curated" / filename
...
def main():
    ...
    # Create shared project memory structure
    print(f"\nCreating shared project memory at {project_memory_path}")
    initiate_project_memory(project_memory_path)
